	...
	<?php if(get_post_format()!='quote'){?>
		...
		<?php 
			...
			<div class="large_thumb thumb_hover">
					<div class="icon_bar for_link">
						<a href='javascript:TellAFriend_OpenForm("TellAFriend_BoxContainer","TellAFriend_BoxContainerBody","TellAFriend_BoxContainerFooter");' title="<?php the_title_attribute(); ?>" class="icon link" rel="<?php the_permalink() ?>"></a>
		...
		<?php } // if(video_embed) ;?>	
			...
			<div class="post_item_inner">
				<div class="post_meta">
					<span class="user"><?php _e('Autor','presslayer');?> <?php the_author_posts_link(); ?></span> 
				...
				<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title_attribute(); ?>" class="button green"><?php _e('Sigue leyendo','presslayer');?></a>
...
<script>
		...
		$container.imagesLoaded( function(){
		  $container.masonry({
			...
			columnWidth: function(containerWidth) {
				var box_width = $j( '#post_grids' ).children('.post_col').width();
				return box_width;
			}
		  });
		});
	...
	$j(document).ajaxError(function(e,xhr,opt){
	  if (xhr.status == 404) $j('#page-nav a').remove();
	});
	
    $j('.icon.hearth').click(function(){
        $j('#TellAFriend_Link').val($j(this).attr('rel'))
    })
});
</script>
