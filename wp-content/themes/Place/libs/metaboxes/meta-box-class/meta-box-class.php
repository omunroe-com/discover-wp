...
class AT_Meta_Box {
  ...
  public function load_scripts_styles() {
    ...
    // Get Plugin Path
	$plugin_path = get_stylesheet_directory_uri() . '/libs/metaboxes/meta-box-class';
  ...
  public function show_field_upload( $field, $meta) {  
    ...
    echo "<input type='text' class='at-upload".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='30' ".( isset($field['style'])? "style='{$field['style']}'" : '' )."/><input title='{$field['name']}' type='button' class='pl_upload_button' value=' Upload '>";
