...
<div id="comments">
	<?php
		if(get_comments_number() == 1) $cm_str = __('Un comentario','presslayer');
		else $cm_str = get_comments_number(). __(' comentarios','presslayer');
	?>
...
$comment_form_fields =  array(
		'author' => '<div class="comment_field comment_form_author"><label for="author">' . __( 'Nombre', 'presslayer' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label><input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="50" ' . $aria_req . ' /></div>', 
		...
		'url'    => '<div class="comment_field comment_form_url '. ( $pl_data['captcha']!='no' ? '' : 'fullwidth' ) .'"><label for="url">' . __( 'Web', 'presslayer' ) . '</label><input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="50" /></div>');
...
$comment_form = array( 
	'fields' => apply_filters( 'comment_form_default_fields', $comment_form_fields),
		...
		'comment_field' => '<div class="comment_field comment_form_comment"><label for="comment">' . __( 'Comentario', 'presslayer' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label><textarea row="10" id="comment" name="comment" ' . $aria_req . '></textarea>' .'</div>',
		'comment_notes_before' => '',
		'comment_notes_after' => '<em class="respond_info">Campos obligatorios (<span class="required">*</span>)</em>',
		'title_reply' => '<span class="block_title_label">'.__('Dejanos tu comentario','presslayer').'</span>',
	); 
